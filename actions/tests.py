...
class AddActionAPIViewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.test_sensor = Sensors.objects.create(name=self.name, category=self.category)

        self.name_driver = 'Pilot'
        self.category_driver = 'remote_control'
        self.test_driver = Drivers.objects.create(name=self.name, category=self.category, data=False)
        ...
        self.days = json.dumps(['1', '2'])
        self.start_event = '12:00'
        self.end_event = '13:00'
        self.action = json.dumps(['1'])
        self.flag = 4
